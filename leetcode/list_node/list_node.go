...
func (l *ListNode[T]) String() string {
	...
	res.WriteString(fmt.Sprintf("%v", l.Val))
	cur := l.Next
	for cur != nil {
...
// NewListNode 构造函数
func NewListNode(val int) *ListNode[int] {
	return &ListNode[int]{Val: val}
}

// SimpleParse 对ParseListNode方法的简单封装，使用默认分隔符解析链表
// param: listExp 链表的表达式
// return: 头结点
func SimpleParse(listExp string) *ListNode[int] {
...
// param: spt 表达式的分割符
// return: 生成的链表的头节点
func ParseListNode(exp, spt string) *ListNode[int] {
	var (
...
// Equal 判断两个链表是否相等
func Equal[T comparable](first, second *ListNode[T]) bool {
